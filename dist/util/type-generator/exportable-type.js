...
class ExportableType {
    ...
    visitWithState(visitor, currentKey, path, state) {
    ...
    updateASTWithJSDoc() {
        return this.collect((path, oldValue, childResults) => {
            ...
            if (node.hasDeclaration && descSources === 1) {
                addJSDoc(node.type, desc);
            }
            return desc;
        });
    }
    addDependency(key, child) {
    ...
    updateTypeReference(key) {
        if (typeof this.type === 'string') {
            ...
            this.type.elementType = this.dependencies_.get(key).createReferenceType();
        }
        else if (ts.isTypeLiteralNode(this.type) || ts.isInterfaceDeclaration(this.type)) {
            // Update property signature with matching name
            for (let i = 0; i < this.type.members.length; i++) {
                let prop = this.type.members[i];
    ...
    static createInterfaceType(title, members, dependencies) {
        let tsType = ts.createInterfaceDeclaration(undefined, [ts.createToken(ts.SyntaxKind.ExportKeyword)], title, undefined, undefined, members);
    ...
    static createTypeLiteral(title, members, dependencies) {
    ...
    static astEnum(schema, title) {
        ...
        if (schema.type === 'boolean') {
            ...
            if (schema.enum.length <= 3) {
                ...
                let values = [];
                for (let v of schema.enum) {
                    values.push(ts.createLiteralTypeNode(ts.createLiteral(v)));
                }
                return ts.createUnionTypeNode(values);
            }
            else {
    ...
    // isTopLevel is true
    static astObject(schema, title, isTopLevel) {
        ...
        for (let name of Object.keys(schema.properties)) {
            ...
            let propType = ExportableType.processDefinition(prop, title + '_' + name, false);
            let tsType;
            ...
            let questionToken = required ? undefined : ts.createToken(ts.SyntaxKind.QuestionToken);
            let signature = ts.createPropertySignature(undefined, name, questionToken, tsType, undefined);
            members.push({ sig: signature, name: name, desc: prop.description || '' });
        }
        ...
        if (isTopLevel || dependencies.size > 0) {
            let complexType;
            if (isTopLevel) {
                complexType = ExportableType.createInterfaceType(title, memberSigs, dependencies);
            }
            else {
                complexType = ExportableType.createTypeLiteral(title, memberSigs, dependencies);
            }
    ...
    static astArray(schema, title) {
        ...
        let element = ExportableType.processDefinition(schema.items, title + '_element', true);
        if (element instanceof ExportableType) {
            ...
            let arrayExport = new ExportableType(title, ts.createArrayTypeNode(element.createReferenceType()));
    ...
    static processDefinition(schemaOrType, title, isTopLevel = true) {
        if (typeof schemaOrType === 'string') {
            ...
            if (schemaOrType.properties) {
                ...
                return ExportableType.processDefinition(schemaOrType.type, title, isTopLevel);
            }
            else {
            ...
            return ExportableType.processDefinition(schemaOrType.schema, title, isTopLevel);
        }
        else {
            return ts.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword);
        }
    }
    static createParameterType(route, root) {
        let routeType = root.dependency(route.id);
        if (!routeType) {
            throw new Error('Route group is undefined after processing spec: ' + route.id);
        }
        ...
        for (let param of route.parameterNames) {
            ...
            if (!paramType) {
                throw new Error('Parameter type is undefined after processing route: ' + route.id
                    + ' -> ' + param);
            }
            if (source === 'query' || source === 'path') {
                ...
                if (bodyParam) {
                    throw new Error('Multiple body parameters in route are not supported: '
                        + route.id);
                }
                else {
                    bodyParam = paramType;
        ...
        if (bodyParam) {
            ...
            aggregateSigs.push(ts.createPropertySignature(undefined, 'body', bodyQuestionToken, bodyParam.createReferenceType(), undefined));
        }
        if (aggregateSigs.length > 0) {
            return ExportableType.createTypeLiteral(route.id + '_params', aggregateSigs, aggregateDeps);
        }
        else {
    ...
    static createParameterTypeAggregate(spec, root) {
        let memberTypes = new Map();
        let members = [];
        for (let route of spec.routeIDs) {
            let paramType = ExportableType.createParameterType(spec.route(route), root);
            let sig;
            if (paramType) {
                memberTypes.set(route, paramType);
                sig = ts.createPropertySignature(undefined, route, undefined, paramType.createReferenceType(), undefined);
            }
            else {
                sig = ts.createPropertySignature(undefined, route, undefined, ts.createKeywordTypeNode(ts.SyntaxKind.UndefinedKeyword), undefined);
            }
            members.push({ sig: sig, route: spec.route(route) });
        }
        members.sort(sortByRoute);
        let params = ExportableType.createInterfaceType('Parameters', members.map(m => m.sig), memberTypes);
        params.addDescription('A special-purpose interface that provides keys mapping from route ID to a structure describing the parameters for the route.  This is not intended to be instantiated, but as a tool with TypeScript\'s `keyof` features to support type checking on generic request functions');
        for (let id of spec.routeIDs) {
            let route = spec.route(id);
            params.addDescription('The type of this member specifies the path, query, and body parameters for the route: [`'
                + route.httpMethod.toUpperCase() + ' ' + route.path + '`]('
                + route.docURL
                + '). If the type does not have a `query`, `path`, or `body` property, then the route does not define parameters for that source type.', id);
        }
        return params;
    }
    static createResponseTypeAggregate(spec, root) {
        let memberTypes = new Map();
        let members = [];
        for (let route of spec.routeIDs) {
            let routeAggregate = root.dependency(route);
            if (!routeAggregate) {
                throw new Error('Route group is undefined after processing spec: ' + route);
            }
            let responseType = routeAggregate.dependency('__response__');
            if (!responseType) {
                throw new Error('Route response type is undefined after processing spec: ' + route);
            }
            let sig = ts.createPropertySignature(undefined, route, undefined, responseType.createReferenceType(), undefined);
            memberTypes.set(route, responseType);
            members.push({ sig: sig, route: spec.route(route) });
        }
        members.sort(sortByRoute);
        let rawResponses = ExportableType.createInterfaceType('Responses', members.map(m => m.sig), memberTypes);
        rawResponses.addDescription('A special-purpose interface that provides keys mapping from route ID to the response type for each route.  This is not intended to be instantiated, but as a tool with TypeScript\'s `keyof` features to support type checking on generic response functions');
        for (let id of spec.routeIDs) {
            let route = spec.route(id);
            rawResponses.addDescription('The type of this member is the response type of for the route: [`'
                + route.httpMethod.toUpperCase() + ' ' + route.path + '`]('
                + route.docURL + ').', id);
        }
        return rawResponses;
    }
    static createRouteURLMap(spec) {
        // First create a value type to hold the URL and the HTTP method.
        let urlSig = ts.createPropertySignature(undefined, 'url', undefined, ts.createKeywordTypeNode(ts.SyntaxKind.StringKeyword), undefined);
        ...
        let methodSig = ts.createPropertySignature(undefined, 'method', undefined, httpMethodType, undefined);
        let urlMapType = ExportableType.createInterfaceType('URLInfo', [urlSig, methodSig], new Map());
        let mapMembers = [];
        let mapDependencies = new Map();
        for (let id of spec.routeIDs) {
            let route = spec.route(id);
            let url = ts.createPropertyAssignment('url', ts.createLiteral(route.path));
            let method = ts.createPropertyAssignment('method', ts.createLiteral(route.httpMethod.toUpperCase()));
            let value = ts.createObjectLiteral([url, method], false);
            mapMembers.push({
                route: route,
                sig: ts.createPropertySignature(undefined, id, undefined, urlMapType.createReferenceType(), undefined),
                value: ts.createPropertyAssignment(id, value)
            });
            mapDependencies.set(id, urlMapType);
        }
        mapMembers.sort(sortByRoute);
        let routeMapType = ExportableType.createInterfaceType('RouteMap', mapMembers.map(m => m.sig), mapDependencies);
        let declaration = ts.createVariableStatement([
            ts.createToken(ts.SyntaxKind.ExportKeyword),
            ts.createToken(ts.SyntaxKind.ConstKeyword)
        ], [
            ts.createVariableDeclaration('ROUTE_MAP', routeMapType.createReferenceType(), ts.createObjectLiteral(mapMembers.map(m => m.value), true))
        ]);
        let routeMap = new ExportableType('ROUTE_MAP', declaration);
        routeMap.addDependency('RouteMap', routeMapType);
        return routeMap;
    }
    static buildTypeGraph(spec) {
        ...
        for (let routeID of spec.routeIDs) {
            ...
            for (let param of route.parameterNames) {
                let type = ExportableType.processDefinition(route.parameterData(param), routeID + '_' + param);
                if (type instanceof ExportableType) {
                    ...
                    routeAggregator.addDependency(param, new ExportableType(routeID + '_' + param, type));
                }
            }
            // Convert response
            ...
            if (response instanceof ExportableType) {
                ...
                routeAggregator.addDependency('__response__', new ExportableType(routeID + suffix, response));
            }
        }
        root.buildMaximalReuseGraph();
        root.addDependency('Responses', ExportableType.createResponseTypeAggregate(spec, root));
        root.addDependency('Parameters', ExportableType.createParameterTypeAggregate(spec, root));
        root.addDependency('ROUTE_MAP', ExportableType.createRouteURLMap(spec));
        root.updateASTWithJSDoc();
        return root;
    }
}
exports.ExportableType = ExportableType;
...
function checkMemberCompatibility(a, b) {
...
function camelCaseToEnumName(camelCase) {
    if (+camelCase) {
...
function isValidDescription(description) {
    return description.toLowerCase() !== description && description.split(' ').length > 2;
}
...
//# sourceMappingURL=exportable-type.js.map