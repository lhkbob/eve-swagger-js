...
const fs = require("fs");
function walkProgram(fileNames, options) {
...
function isType(object) {
    return object['getSymbol'] !== undefined;
}
let knownTypes = new Map();
let typeIDSeq = 1;
function getType(nodeOrType, checker) {
    let type = undefined;
    if (nodeOrType === undefined) {
        ...
        else if (nodeOrType.name && nodeOrType.flags) {
            try {
...
function visitChildren(node, checker) {
    let children = [];
    node.forEachChild(c => {
...
function visitNode(node, checker) {
    ...
    if (ts.isClassDeclaration(node) && isExported(node)) {
        ...
        if (ts.isPropertyAccessExpression(caller)) {
            ...
            if (propertyName == 'request') {
                let callerTypeName = checker.typeToString(checker.getTypeAtLocation(caller.expression));
                if (callerTypeName == 'ESIAgent') {
                    // HACK for now
                    let route = node.arguments[0].text;
                    return {
...
//# sourceMappingURL=node-walk.js.map