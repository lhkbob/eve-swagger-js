/**
 * A container for the [fleets](https://esi.tech.ccp.is/latest/#/Fleets) ESI
...
module.exports = function(api) {
   ...
   *   the request. If not provided, the default access token provided to the
   *   factory is used; if that was not set then this request will fail.
   * @param {String} language Localization code for the response, which
   *   will override the default configured localization
   * @return {external:Promise} A Promise that resolves to the response of
   *   the request
  ...
  exports.addWing = function(id, accessToken) {
    return newRequest(ESI.FleetsApi, 'postFleetsFleetIdWings', [id],
        accessToken);
  };

  /**
   * Remove a wing from a fleet with the ESI endpoint. This makes an HTTP DELETE
   * request to
   ...
   * @param {Integer} wingId The wing to update
  ...
  /**
   * Add a new sequad to a fleet's wing with the ESI endpoint. This makes an
