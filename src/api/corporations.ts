...
export function makeCorporations(agent: ESIAgent): Corporations {
  let functor = <Corporations> <any> function (id: number) {
    return new CorporationImpl(agent, id);
  };
  functor.search = makeDefaultSearch(agent, esi.SearchCategory.CORPORATION);
  ...
  functor.names = function (ids: number[]) {
    if (ids.length > 100) {
      ...
      return agent.request('get_corporations_names',
 ...
 *    not needed
 */
export function makeCorporation(agent: ESIAgent, id: number): Corporation {
  return new CorporationImpl(agent, id);
}
...
class CorporationImpl implements Corporation {
  ...
  info() {
    return this.agent.request('get_corporations_corporation_id',
        { path: { 'corporation_id': this.id_ } });
  }

  history() {
    return this.agent.request('get_corporations_corporation_id_alliancehistory',
        { path: { 'corporation_id': this.id_ } });
  }

  icon() {
    return this.agent.request('get_corporations_corporation_id_icons',
        { path: { 'corporation_id': this.id_ } });
  }

  loyaltyOffers() {
    return this.agent.request('get_loyalty_stores_corporation_id_offers',
        { path: { 'corporation_id': this.id_ } });
  }
