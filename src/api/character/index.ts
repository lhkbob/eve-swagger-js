...
export function makeCharacters(agent: ESIAgent): Characters {
  const charSearch = makeDefaultSearch(agent, esi.SearchCategory.CHARACTER);

  return <Characters> function (ids: number | number[] | Set<number> | string,
    ...
    if (typeof ids === 'number') {
      ...
      if (typeof strictOrToken === 'string') {
        return new AuthenticatedCharacter(
            { agent, ssoToken: strictOrToken, id: ids });
      } else {
        return new Character(agent, ids);
      }
    } else if (typeof ids === 'string') {
      // Search variant (which only permits the second argument to be a boolean)
      return new MappedCharacters(agent,
          () => charSearch(ids, <boolean> strictOrToken));
    } else {
      // ids is a set or array so just a plain mapped corp
      return new MappedCharacters(agent, ids);
    }
  };
}