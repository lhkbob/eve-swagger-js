/**
 * A container for the [calendar](https://esi.tech.ccp.is/latest/#/Calendar) ESI
...
module.exports = function(api) {
  ...
  exports.get = function(characterId, accessToken, fromEventId) {
    ...
    return newRequestOpt(ESI.CalendarApi, 'getCharactersCharacterIdCalendar',
  ...
  exports.getEvent = function(characterId, eventId, accessToken) {
    return newRequest(ESI.CalendarApi,
        'getCharactersCharacterIdCalendarEventId', [characterId, eventId],
        accessToken);
  };

  /**
   * Update response for a specific event via the ESI end point. This makes an
   * HTTP PUT request to
   ...
   * The request is returned as an asynchronous Promise that resolves to an
   * empty object on success. Allowed response values can be
   *
   * 1. `'rejected'`
   * 2. `'accepted'`
   * 3. `'undecided'`
  ...
  exports.updateResponse = function(characterId, eventId, response,
    ...
    return newRequest(ESI.CalendarApi,
        ...
        [characterId, eventId, { response: response }], accessToken);
  };
