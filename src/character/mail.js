/**
 * A container for the [mail](https://esi.tech.ccp.is/latest/#/Mail) ESI
...
module.exports = function(api) {
  ...
  /**
   * Calculate the CSPA mail prices for sending mail from the character `fromId`
   * to the characters in `toIds`, via the ESI endpoint. This makes an HTTP POST
   * request to
   * [`/character/{fromId}/cspa/`](https://esi.tech.ccp.is/latest/#!/Character/post_characters_character_id_cspa).
  ...
  exports.getInbox = function(id, labelIds, lastMailId, accessToken) {
    ...
    return newRequestOpt(ESI.MailApi, 'getCharactersCharacterIdMail', [id],
        opts, accessToken);
  };

  /**
   * Send a mail from the given character `id`, where the mail contents and
   * destination are described by `mail`. This makes an HTTP POST request to
  ...
  exports.send = function(id, mail, accessToken) {
    return newRequest(ESI.MailApi, 'postCharactersCharacterIdMail', [id, mail],
        accessToken);
  };
  ...
  exports.getInboxLabels = function(id, accessToken) {
    return newRequest(ESI.MailApi, 'getCharactersCharacterIdMailLabels', [id],
        accessToken);
  };

  /**
   * Create a new inbox label for the given character `id`. This makes an HTTP
   * POST request to
   * [`/characters/{id}/mail/labels`](https://esi.tech.ccp.is/latest/#!/Mail/post_characters_character_id_mail_labels).
   * The request is returned as an asynchronous Promise that resolves to an
   * Integer on success representing the new label's id. An example `label`
   * value looks like:
   *
   * ```
   * {
  ...
  /**
   * Remove the label identified by `labelId` for the given `characterId`. This
   ...
   *
   * @param {Integer} characterId The character id the whose mailbox has the
   *     label deleted from
  ...
  exports.removeInboxLabel = function(characterId, labelId, accessToken) {
    return newRequestOpt(ESI.MailApi,
        'deleteCharactersCharacterIdMailLabelsLabelId', [id], { label: label },
        accessToken);
  };

  /**
   * Get all of the the character's mailing list memberships from the ESI end
  ...
  exports.getMailingLists = function(id, accessToken) {
    return newRequest(ESI.MailApi, 'getCharactersCharacterIdMailLists', [id],
        accessToken);
  };

  /**
   * Delete the given mail from the character's inbox via the ESI endpoint. This
   * makes an HTTP DELETE request to
